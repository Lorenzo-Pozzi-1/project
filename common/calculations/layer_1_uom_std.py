...
class EIQUOMStandardizer:
    ...
    def standardize_single_ai_inputs(self, 
        ...
        print(f"\nL1 - standardize_single_ai_inputs: Starting standardization with AI EIQ={ai_eiq}, concentration={ai_concentration} {ai_concentration_uom}, rate={application_rate} {application_rate_uom}, applications={applications}\n")
        ...
        print("Step 2 - Standardizing AI concentration to match rate units")
        ai_concentration_per_unit = self._standardize_ai_concentration(
            ai_concentration, ai_concentration_uom, rate_unit_type
        )
        print(f"AI concentration standardized to {ai_concentration_per_unit}")
        
        # Step 3: Standardize EIQ to [eiq/kg] (Cornell is eiq/lb)
        print("Step 3 - Standardizing AI EIQ from Cornell units")
        ai_eiq_per_kg = self._standardize_ai_eiq(ai_eiq)
        print(f"AI EIQ standardized to {ai_eiq_per_kg} eiq/kg")
        
        # Step 4: Validate dimensional analysis
        print("Step 4 - Validating dimensional analysis")
        ...
        print(f"Returning standardized inputs: rate={result.rate_per_ha}, type={result.rate_unit_type}, concentration={result.ai_concentration_per_unit}, eiq={result.ai_eiq_per_kg}")
        return result
    ...
    def standardize_product_inputs(self,
        ...
        print(f"\nL1 - standardize_product_inputs: Starting product standardization with {len(active_ingredients)} active ingredients, rate={application_rate} {application_rate_uom}, applications={applications}\n")
        
        # Step 1: Standardize application rate
        print("Step 1 - Standardizing product application rate")
        rate_per_ha, rate_unit_type = self._standardize_application_rate(
            application_rate, application_rate_uom, user_preferences
        )
        print(f"Product application rate standardized to {rate_per_ha} with unit type '{rate_unit_type}'")
        
        # Step 2: Standardize all active ingredients to match rate units
        print("Step 2 - Standardizing all active ingredients")
        standardized_ais = []
        for i, ai in enumerate(active_ingredients):
            print(f"Processing AI {i+1}/{len(active_ingredients)}: {ai.get('name', 'Unknown')}")
            ...
            try:
                print(f"Calling standardize_single_ai_inputs for AI {ai.get('name', 'Unknown')}")
                ...
                print(f"Successfully standardized AI {ai.get('name', 'Unknown')}: concentration={ai_data['concentration_per_unit']}, eiq={ai_data['eiq_per_kg']}")
        ...
        print(f"Returning product inputs with {len(standardized_ais)} standardized AIs")
        return result
    ...
    def _standardize_application_rate(self, 
        ...
        """
        print(f"\tL1 - _standardize_application_rate: Converting rate {rate} {rate_uom} to standard units")
        
        if not rate or not rate_uom:
            raise ValueError("Application rate and UOM are required")
        
        print(f"\tL1 - _standardize_application_rate: Parsing composite UOM from '{rate_uom}'")
        from_uom = CompositeUOM(rate_uom) # Parse the UOM into numerator / denominator
        print(f"\tL1 - _standardize_application_rate: Parsed UOM - numerator: {from_uom.numerator}, denominator: {from_uom.denominator}")
        
        # Determine target unit (kg or l) based on numerator type
        print(f"\tL1 - _standardize_application_rate: Getting base unit for numerator '{from_uom.numerator}'")
        ...
        print(f"\tL1 - _standardize_application_rate: Numerator unit category is '{numerator_unit.category}'")
        
        if numerator_unit.category == 'weight':
            target_uom = CompositeUOM("kg/ha")
            unit_type = "weight"
            print("\tL1 - _standardize_application_rate: Target unit determined as kg/ha (weight-based)")
        elif numerator_unit.category == 'volume':
            target_uom = CompositeUOM("l/ha") 
            unit_type = "volume"
            print("\tL1 - _standardize_application_rate: Target unit determined as l/ha (volume-based)")
        else:
            raise ValueError(f"Application rate must be weight/area or volume/area, got: {rate_uom}")
        
        # Convert to standard rate
        print(f"\tL1 - _standardize_application_rate: Converting {rate} from {from_uom.original_string} to {target_uom.original_string}")
        standardized_rate = self.uom_repo.convert_composite_uom(
            rate, from_uom, target_uom, user_preferences
        )
        print(f"\tL1 - _standardize_application_rate: Conversion complete - {rate} {rate_uom} = {standardized_rate} {target_uom.original_string}")
    ...
    def _standardize_ai_concentration(self, concentration: float, concentration_uom: str, target_rate_type: str) -> float:
        ...
        """
        print(f"\tL1 - _standardize_ai_concentration: Converting concentration {concentration} {concentration_uom} to match rate type '{target_rate_type}'")
        ...
        if concentration_uom == '%':
            ...
            print(f"\tL1 - _standardize_ai_concentration: Converting percentage - {concentration}% = {result} (decimal)")
            return result
        
        print(f"\tL1 - _standardize_ai_concentration: Parsing composite UOM from '{concentration_uom}'")
        from_uom = CompositeUOM(concentration_uom) # Parse the UOM into numerator / denominator
        print(f"\tL1 - _standardize_ai_concentration: Parsed concentration UOM - numerator: {from_uom.numerator}, denominator: {from_uom.denominator}")
        ...
        if target_rate_type == "weight":
            ...
            print("\tL1 - _standardize_ai_concentration: Target concentration unit set to kg/kg (weight-based)")
        elif target_rate_type == "volume":
            target_uom = CompositeUOM("kg/l")   # [kg AI / l product]
            print("\tL1 - _standardize_ai_concentration: Target concentration unit set to kg/l (volume-based)")
        else:
        ...
        if from_uom.is_concentration:
            print("\tL1 - _standardize_ai_concentration: Checking physical compatibility of concentration units")
            ...
            print(f"\tL1 - _standardize_ai_concentration: Source is weight/volume: {from_is_weight_per_volume}, weight/weight: {from_is_weight_per_weight}")
            ...
            if from_is_weight_per_weight and target_rate_type == "volume":
                ...
                print(f"\tL1 - _standardize_ai_concentration: WARNING! Converting concentration from {concentration_uom} to kg/l assuming density ~1 kg/l")
        
        # Convert concentration
        try:
            print(f"\tL1 - _standardize_ai_concentration: Converting {concentration} from {from_uom.original_string} to {target_uom.original_string}")
            standardized_concentration = self.uom_repo.convert_composite_uom(
                concentration, from_uom, target_uom
            )
            print(f"\tL1 - _standardize_ai_concentration: Conversion complete - {concentration} {concentration_uom} = {standardized_concentration} {target_uom.original_string}")
            return standardized_concentration
        except Exception as e:
            print(f"\tL1 - _standardize_ai_concentration: Conversion failed with error: {e}")
            raise ValueError(
                f"\tL1._standardize_ai_concentration: Cannot convert concentration from {concentration_uom} to {target_uom.original_string}: {e}"
            )
    ...
    def _standardize_ai_eiq(self, ai_eiq: float) -> float:
        ...
        """
        print(f"\tL1 - _standardize_ai_eiq: Converting AI EIQ {ai_eiq} from Cornell units (eiq/lb) to standard (eiq/kg)")
        
        if ai_eiq is None:
            print("\tL1 - _standardize_ai_eiq: WARNING! Missing AI eiq value, calculating assuming it is 0.")
            return 0.0
        
        if ai_eiq == 0:
            print("\tL1 - _standardize_ai_eiq: AI EIQ is 0, returning 0.0")
            return 0.0
        
        # Cornell EIQ is per pound, convert to per kg
        # Since 1 lb = 0.453592 kg, we divide the EIQ/lb value by this factor to get EIQ/kg
        print("\tL1 - _standardize_ai_eiq: Converting from lb to kg using UOM repository")
        ...
        print(f"\tL1 - _standardize_ai_eiq: Conversion complete - {ai_eiq} eiq/lb = {result} eiq/kg")
        return result
    ...
    def _validate_dimensional_analysis(self, rate_unit_type: str, ai_concentration: float):
        ...
        """
        print(f"\tL1 - _validate_dimensional_analysis: Validating dimensional analysis for rate type '{rate_unit_type}' with concentration {ai_concentration}")
        
        if rate_unit_type not in ["weight", "volume"]:
            raise ValueError(f"Invalid rate unit type: {rate_unit_type}")
        
        if ai_concentration <= 0:
            print(f"\tL1 - _validate_dimensional_analysis: ERROR - AI concentration {ai_concentration} must be positive")
        ...
        if rate_unit_type == "weight" and ai_concentration > 1.0:
            print(f"\tL1 - _validate_dimensional_analysis: ERROR - Weight-based concentration {ai_concentration} cannot exceed 1.0 (100%)")
            raise ValueError("Layer1._validate_dimensional_analysis: Weight-based concentration cannot exceed 1.0 (100%)")
        
        print("\tL1 - _validate_dimensional_analysis: All dimensional analysis checks passed")
        
    def _is_weight_per_volume(self, uom: CompositeUOM) -> bool:
        """Check if UOM is weight per volume (like lb/gal, g/l)."""
        print(f"\tL1 - _is_weight_per_volume: Checking if {uom.original_string} is weight per volume")
        
        if not uom.is_concentration:
            print("\tL1 - _is_weight_per_volume: UOM is not a concentration, returning False")
            return False
        ...
        print(f"\tL1 - _is_weight_per_volume: Result for {uom.original_string}: {is_weight_per_volume}")
    ...
    def _is_weight_per_weight(self, uom: CompositeUOM) -> bool:
        ...
        print(f"\tL1 - _is_weight_per_weight: Checking if {uom.original_string} is weight per weight")
        
        if uom.numerator == '%':
            print("\tL1 - _is_weight_per_weight: UOM is percentage, returning True")
            return True
            
        if not uom.is_concentration:
            print("\tL1 - _is_weight_per_weight: UOM is not a concentration, returning False")
            return False
        ...
        print(f"\tL1 - _is_weight_per_weight: Result for {uom.original_string}: {is_weight_per_weight}")
