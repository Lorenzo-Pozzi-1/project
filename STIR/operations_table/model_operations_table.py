...
class STIROperationsTableModel(QAbstractTableModel):
    ...
    def __init__(self, parent=None):
        ...
        self._custom_machine_repo = CustomMachineRepository.get_instance()
            ...
            if column == 0:  # Group
                ...
                if new_machine_name != operation.machine_name:
                    if new_machine_name:
                        ...
                        if machine:
                            # Update operation with regular machine parameters
                            ...
                            # Clear custom machine data
                            operation.custom_machine_tools = None
                            operation.calculate_stir()
                        else:
                            # Check if it's a custom machine
                ...
                # Convert from display UOM to cm (standard)
                ...
                if operation.is_custom_machine():
                    # For custom machines, adjust all tools proportionally
                    if operation.custom_machine_tools:
                        current_max_depth = max(tool.depth for tool in operation.custom_machine_tools)
                        if current_max_depth > 0 and abs(depth_cm - current_max_depth) > 0.01:
                            operation.adjust_custom_machine_depth(depth_cm, "cm")
                            operation.calculate_stir()
                            self.dataChanged.emit(index, index, [Qt.DisplayRole])
                            self._emit_signals()
                            return True
                else:
                    # Regular machine - convert cm to operation's stored UOM
                ...
                # Custom machines have read-only area disturbed (calculated from tools)
                if operation.is_custom_machine():
                    return False  # Not editable for custom machines
                
                # Regular machines - allow editing
    ...
    def flags(self, index: QModelIndex) -> Qt.ItemFlags:
        ...
        if row_type == self.ROW_TYPE_OPERATION:
            ...
            # Check if this is a custom machine operation
            ...
            if index.column() <= 5:  # Columns 0-5 are potentially editable, 6 is read-only
                ...
                if is_custom and index.column() == 4:
                    ...
                    pass  # Don't add ItemIsEditable flag
                else:
    ...
    def _get_operation_data(self, index: QModelIndex, role: int, row_info: Dict) -> Any:
        ...
        if role == Qt.DisplayRole or role == Qt.EditRole:
            if column == 0:  # Group (indented for visual hierarchy)
                ...
                # For custom machines, display maximum depth across all tools
                if operation.is_custom_machine():
                    if operation.custom_machine_tools:
                        max_depth = max(tool.depth for tool in operation.custom_machine_tools)
                        if max_depth > 0:
                            # Convert from cm to display UOM
                            if self.display_depth_uom.lower() == 'inch':
                                display_value = max_depth / 2.54
                            else:
                                display_value = max_depth
                            return f"{display_value:.1f}"
                    return "0.0"
                else:
                    # Regular machine - display depth in the selected UOM
